...
def mo (ranges, dt):
    for range in ranges:
        for month in MONTHS.keys ():
            range = re.sub (r"%s.*?(?=\s|-|$)" % (month), str (MONTHS[month]), range)
...
def wd (ranges, dt):
    ...
    for range in ranges:
        ...
        for day in DAYS.keys ():
            range = re.sub (r"%s.*?(?=\s|-|$)" % (day), str (DAYS[day]), range)
...
def _in_min_max (low, high, min_val, max_val, scale):
    ...
    if low < min_val or low > max_val:
        raise InvalidFormat ("%d is not valid for %s. Valid options are between %d and %d." % (low, scale, min_val, max_val))
    if high != None:
        try:
            high = int (high)
        except ValueError:
