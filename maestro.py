...
"""
Maestro - A job execution engine with dependency support
...
class Config:
    """Configuration constants for the maestro."""
...
def db_connection():
    ...
    try:
        ...
        logging.getLogger('maestro').error(f"Database error: {e}")
        raise
    finally:
        if conn:
            try:
                ...
                logging.getLogger('maestro').error(f"Error closing database connection: {e}")
...
class JobExecutioner:
    ...
    def _load_dependency_plugins(self):
        """Load dependency plugins specified in the configuration."""
        import importlib.util
        try:
            # Define a class to track resolvers if not already defined
            ...
            for plugin_path in self.dependency_plugins:
                ...
                try:
                    ...
                    # Check if the module has a DependencyResolver class
    ...
    def _setup_logging(self):
        ...
        # Create a master log with run_id in the filename
        # Format: maestro.{app_name}.run-{run_id}.log
        master_log_path = os.path.join(Config.LOG_DIR, f"maestro.{self.application_name}.run-{self.run_id}.log")
        
        # Also maintain a general log for the application (without run_id)
        # Format: maestro.{app_name}.log
        app_log_path = os.path.join(Config.LOG_DIR, f"maestro.{self.application_name}.log")
        ...
        # Add run_id as an extra attribute to the logger's LogRecord
    ...
    def _get_new_run_id(self) -> int:
        """Get a new run ID from the database."""
        try:
            ...
            # Don't use self.logger here since it might not be initialized yet
    ...
    def _get_previous_run_status(self, resume_run_id: int) -> Dict[str, str]:
        """Get job statuses from a previous run."""
        job_statuses = {}
        try:
            with db_connection() as conn:
                cursor = conn.cursor()
    ...
    def _update_job_status(self, job_id: str, status: str):
        """Queue job status for batch database update."""
        if self.dry_run:
            return
    ...
    def _commit_job_statuses(self):
        """Commit batched job status updates to the database."""
    ...
    def _setup_job_logger(self, job_id: str) -> Tuple[logging.Logger, logging.FileHandler, str]:
        """Set up a dedicated logger for a job without propagating to the parent logger."""
        # Create a job-specific log file with run_id in the name
        # Format: maestro.{app_name}.job-{job_id}.run-{run_id}.log
        job_log_path = os.path.join(Config.LOG_DIR, f"maestro.{self.application_name}.job-{job_id}.run-{self.run_id}.log")
        
        # Set up the job logger
        job_logger = logging.getLogger(f'job_{job_id}')
        
        # Prevent propagation to parent loggers (this is the key change)
        ...
        # Create and configure a file handler for the job
        ...
        # Log job execution start in both job log and main application log
        ...
        # Log a summary message in the main application log
    ...
    def _validate_command(self, command: str, job_id: str, job_logger: logging.Logger) -> Tuple[bool, str]:
        """
        Validate a command for security issues with enhanced detection.
        
        Performs comprehensive security checks on the command string:
        1. Command injection vulnerability checks
        2. Shell escape sequence detection
        3. Dangerous system operation pattern matching
        4. Allowlist pattern matching
        5. Command tokenization and individual argument analysis
        ...
        security_level = self.config.get("security_level", "medium")  # low, medium, high
        
        # Check command whitelist first (fastest check)
        command_whitelist = self.config.get("command_whitelist", [])
        if command_whitelist:
            # Normalize the command string for whitelist matching
        ...
        # Check command against allowed workspace paths if configured
        workspace_paths = self.config.get("workspace_paths", [])
        
        # Attempt to parse the command for more thorough analysis
        try:
            # Try to tokenize the command using shlex
            tokens = shlex.split(command)
            if tokens:
                # Check for suspicious paths or arguments
                binary_name = tokens[0]
                if workspace_paths and binary_name.startswith('/'):
                    # Check if command binary is within allowed paths
                ...
                for arg in tokens[1:]:
                    ...
                    # Look for sensitive files access
            ...
            # If we can't parse the command, it might have unbalanced quotes or other issues
            job_logger.warning(f"Command parsing failed: {e} - treating with caution")
            # Continue with pattern matching since we can't parse tokens
        
        # Critical patterns - these are always blocked regardless of security level
        critical_patterns = [
        ...
        # Check for critical security issues - always blocked regardless of policy
        ...
        for pattern, description in check_patterns:
            if re.search(pattern, command, re.IGNORECASE):
                ...
                # For "warn" policy, just log the warning and continue
                return True, reason
    ...
    def _run_subprocess_command(self, command: str, job_id: str, job_logger: logging.Logger, timeout: int) -> bool:
        """Run a subprocess command with enhanced security and cleanup."""
        process = None
        output_queue = Queue()
        ...
        try:
            ...
            # Handle unsafe commands based on security policy
            security_policy = self.config.get("security_policy", "warn")
            if not is_safe:
            ...
            # Log a warning if command is potentially unsafe but allowed to run
            if reason:
            ...
            # Determine command execution method
            if parsed_command and not parsed_command.get('needs_shell', True):
                # We can safely execute without shell
                ...
                # Fallback to shell execution if necessary or if command parsing failed
                if not self.allow_shell:
                    job_logger.error("Shell execution required but disabled by configuration (allow_shell=False)")
                    self._update_job_status(job_id, "ERROR")
                    return False
                
                # Check for shell_safe parameter which overrides the parsing result
                shell_features_needed = parsed_command.get('needs_shell', True) if parsed_command else True
                
                # Log shell usage for clarity and security awareness
                ...
                process = subprocess.Popen(
                    command,  # Use the command directly without quoting
                    shell=True,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT,
                    universal_newlines=True,
            ...
            try:
                # Ensure signal module is properly imported even if this runs before line 1134
                import signal
                ...
                # Log timeout in application log
            ...
            if process.returncode == 0:
                ...
                # Log success summary in application log
                self.logger.info(f"Job '{job_id}' completed successfully")
                return True
            else:
                job_logger.error(f"Job {job_id}: FAILED with exit code {process.returncode}")
                self._update_job_status(job_id, "FAILED")
                # Only log failures in application log, as they're important to highlight
            ...
            if process and process.poll() is None:
                self._terminate_process(process, job_logger)
            self._update_job_status(job_id, "ERROR")
            # Continue to log exceptions in application log as they're important to highlight
            self.logger.error(f"Job '{job_id}' failed with exception: {e}")
            return False
    
    def _parse_command(self, command: str, logger: logging.Logger) -> dict:
        """
        Parse a command string intelligently to determine if shell is needed and extract arguments.
        
        Returns:
            dict: A dictionary with the following keys:
                - args: List of command arguments (if shell=False can be used)
                - needs_shell: Boolean indicating if shell execution is required
                - shell_reason: String explaining why shell is needed (if applicable)
        """
        if not command or not command.strip():
            return {'args': [], 'needs_shell': False}
            
        # Check for shell metacharacters that indicate shell features are needed
        shell_indicators = {
            '|': 'pipe',
        ...
        # Shell commands that typically need shell features
        shell_commands = [
            'grep', 'awk', 'sed', 'find', 'xargs',
            'for ', 'while ', 'if ', 'case ', 'do ', 'done',
            'until ', 'function ', 'alias ', 'source ', './'
        ]
        
        # Check if command contains any shell indicators
        ...
        # Check if command starts with shell commands
        ...
        # If no shell features detected, try to split the command safely
        try:
            ...
            # Command is the first element, check if it exists and is executable
            cmd_name = args[0]
            
            # Additional safety check for relative paths
            if '/' in cmd_name:
                # If using relative or absolute path, verify it exists
                if not os.path.isfile(cmd_name) or not os.access(cmd_name, os.X_OK):
                    # If not directly executable, it might need PATH lookup through shell
            ...
            # If we got here, command can be executed without shell
            return {
                'args': args,
                'needs_shell': False,
                'original_command': command
            ...
            # shlex.split failed, likely due to unbalanced quotes
    ...
    def _terminate_process(self, process: subprocess.Popen, job_logger: logging.Logger):
        """Safely terminate a subprocess and its group."""
        try:
            if 'posix' in os.name:
    ...
    def _execute_job(self, job_id: str) -> bool:
        """Execute a job and handle its outcome with automatic retry support."""
        job = self.jobs[job_id]
        command = job["command"]
        timeout = job.get("timeout", Config.DEFAULT_TIMEOUT)
        
        # Set up job logger early to ensure it's available for all log messages
        if self.dry_run:
            job_logger = None
        ...
        # Get retry configuration with enhanced defaults
        max_retries = job.get("max_retries", self.config.get("default_max_retries", 0))
        retry_delay = job.get("retry_delay", self.config.get("default_retry_delay", 30))  # seconds
        retry_backoff = job.get("retry_backoff", self.config.get("default_retry_backoff", 1.5))  # multiplier
        retry_on_status = job.get("retry_on_status", ["ERROR", "FAILED", "TIMEOUT"])
        # New parameters for more advanced retry strategy
        max_retry_time = job.get("max_retry_time", self.config.get("default_max_retry_time", 1800))  # 30 minutes max
        jitter = job.get("retry_jitter", self.config.get("default_retry_jitter", 0.1))  # 10% randomness
        retry_on_exit_codes = job.get("retry_on_exit_codes", self.config.get("default_retry_on_exit_codes", [1]))
        
        # Get retry count and history from database if resuming a previous run
        current_retry = 0
        retry_history = []
        try:
            with db_connection() as conn:
                ...
                try:
                    ...
                    if row:
                        ...
                        if row[1]:  # If retry history exists in DB
                            try:
                                retry_history = json.loads(row[1])
                            except (json.JSONDecodeError, TypeError):
                                # If retry history is corrupt, just start with an empty history
                                retry_history = []
            ...
            # Use class logger instead of job_logger if in dry run mode or if there was an error setting up job_logger
            if job_logger:
            ...
            # Continue with retry_count=0 if we can't get it from DB
            pass
        ...
        try:
            ...
            # Track all attempts with details for analysis
            ...
            while True:
                ...
                if jitter > 0 and attempt > 1:
                    # Add up to ±jitter% randomness to the delay
                    jitter_seconds = random.uniform(-jitter, jitter) * current_delay
                    actual_delay = max(0.1, current_delay + jitter_seconds)  # Ensure at least 100ms delay
                    if abs(jitter_seconds) > 0.1:  # Only log if jitter made a noticeable difference
                ...
                # Store job metrics and retry history in database
                if not self.dry_run:
                    try:
                        with db_connection() as conn:
                ...
                # Handle failure - should we retry?
                should_retry = False
                retry_reason = None
                
                # We know the job failed if we're here, so use that directly
                # instead of checking the database which might not be updated yet
                last_status = "FAILED"  # Assume failed status for retry decision
                
                # Always retry on failure if max_retries is set
                if current_retry < max_retries:
                    should_retry = True
                    retry_reason = f"Job failed and current_retry ({current_retry}) < max_retries ({max_retries})"
                
                # Also check status-based retry conditions for backward compatibility
                if last_status in retry_on_status:
                    should_retry = True
                    retry_reason = f"Status '{last_status}' is in retry_on_status list"
                
                # Check exit code-based retry conditions
                ...
                if jitter > 0:
                    ...
                    current_delay = max(0.1, base_delay * jitter_factor)  # Ensure at least 100ms delay
                else:
                    current_delay = base_delay
                
                job_logger.info(f"Will retry job {job_id} in {current_delay:.1f} seconds (attempt {attempt+1}/{max_retries+1})")
                
                # Update retry info in database with detailed retry history
                if not self.dry_run:
                    self._update_retry_history(job_id, retry_history, current_retry, 
    ...
    def _get_job_status(self, job_id: str) -> str:
        """Get the current status of a job from the database."""
        if self.dry_run:
            return "UNKNOWN"
    ...
    def _update_retry_history(self, job_id: str, retry_history: list, retry_count: int, 
                              status: str, reason: str = None) -> None:
        """Update job retry history in the database with enhanced information."""
        try:
            with db_connection() as conn:
                retry_history_json = json.dumps(retry_history)
                cursor = conn.cursor()
                
                # First check if retry_history column exists
                ...
                # Make sure we have a row for this job
                cursor.execute(
                    "SELECT 1 FROM job_history WHERE run_id = ? AND id = ?",
                    (self.run_id, job_id)
                )
                if not cursor.fetchone():
                    # Create an initial entry if it doesn't exist
                    job = self.jobs[job_id]
                    cursor.execute(
                        """
                ...
                try:
                    ...
                    # In rare cases of column still missing, just log and continue
    ...
    def _get_last_exit_code(self, job_id: str) -> int:
        """Retrieve the last exit code for a job from the database."""
        try:
            with db_connection() as conn:
                cursor = conn.cursor()
                ...
                if 'last_exit_code' not in columns:
                    ...
                    # Return None since the column was just added and has no data
                    return None
                
                # Now try to get the exit code
                try:
                    cursor.execute(
    ...
    @classmethod
    def generate_test_coverage_report(cls, output_file: str = None, fail_under: float = 70.0, 
                                     include_branches: bool = True, ci_mode: bool = False):
        """
...
def test_job_maestro_initialization():
    ...
    try:
        ...
        maestro = JobExecutioner(config_path)
        assert isinstance(maestro, JobExecutioner)
        assert "test1" in maestro.jobs
    finally:
...
"""
    ...
    def _run_dry(self, resume_run_id=None, resume_failed_only=False):
        """Simulate job execution for dry run."""
    ...
    def _get_execution_order(self):
        """Calculate job execution order based on dependencies."""
        order = []
        visited = set()
        temp_visited = set()
    ...
    def run(self, continue_on_error: bool = False, dry_run: bool = False,
            skip_jobs: list = None, max_iter: int = 1000,
            resume_run_id: int = None, resume_failed_only: bool = False):
        ...
        if not self.dry_run:
            ...
            duration = self.end_time - self.start_time
            duration_str = str(duration).split('.')[0]
            # Execution summary will be displayed in the formatted output below
            # No need to log these messages separately
    ...
    def _run_parallel(self, max_iter: int) -> int:
        """Run jobs in parallel with dependency tracking.

        Uses a condition variable to avoid busy-waiting and batches operations
        to reduce lock contention. Ensures thread safety and proper cleanup.
        """
        iteration_count = 0
        ...
        while (not self.job_queue.empty() or pending_futures) and iteration_count < max_iter and not self.interrupted:
            ...
            # Wait for progress if no jobs were queued or completed
    ...
    def _mark_dependent_jobs_failed(self, failed_job_id: str):
        """Mark dependent jobs as failed to prevent execution."""
        with self.lock:
            visited = set()
            queue = [failed_job_id]
            while queue:
    ...
    def _queue_dependent_jobs(self, completed_job_id: str):
        ...
        with self.lock:
            ...
            if jobs_to_queue:
                self.logger.debug(f"Queued {len(jobs_to_queue)} jobs: {', '.join(jobs_to_queue)}")
            with self.job_completed_condition:
                self.job_completed_condition.notify_all()
...
def init_db():
    ...
    # Fast path: Check if database is already initialized
    # This avoids contention when multiple processes try to initialize
    ...
    if db_exists:
        try:
            with db_connection() as conn:
                ...
                if cursor.fetchone():
                    # Check version to provide informative message
            ...
            # If we can't query the database, it might be corrupted or locked
    ...
    try:
        with db_connection() as conn:
            ...
            # Set a reasonable busy timeout to handle contention
            # This makes SQLite wait instead of failing immediately when the database is locked
            cursor.execute("PRAGMA busy_timeout = 5000")  # 5 seconds
            
            # Enable foreign key constraints
            cursor.execute("PRAGMA foreign_keys = ON")
            
            # Begin exclusive transaction to prevent other processes from initializing simultaneously
            # This serves as our locking mechanism
            cursor.execute("BEGIN IMMEDIATE TRANSACTION")
            
            # Create advisory locking table for tracking initialization state
            cursor.execute("""
            ...
            cursor.execute("""
                ...
                VALUES (1, ?, ?)
            """, ("maestro-init", os.getpid()))
            
            # Create schema_version table with enhanced fields if it doesn't exist
            cursor.execute("""
            ...
            # Define migrations with version numbers, descriptions, data migrations, and rollback info
            migrations = [
                ...
                # Version 3: Add execution metrics
                ...
                # Version 4: Add migration_history table for tracking migration events
                ...
                # Add new migrations here with increasing version numbers
            ]
            
            # Create a record of the migration attempt
            migration_id = None
            ...
            try:
                for version, description, statements, data_migration, rollback in migrations:
                    if version > current_version:
                        ...
                        # Calculate a hash of the migration statements for tracking
                        ...
                        for statement in statements:
                            try:
                                ...
                                # Handle column already exists errors for SQLite
                                if "duplicate column name" not in str(e):
                                    # Record the error
                                    if migration_id:
                        ...
                        # Run data migration function if provided
                        if data_migration:
            ...
            # Final commit - this will release our initialization lock
            conn.commit()
        ...
        # Try to roll back any active transaction to release locks
        try:
...
"""
    ...
    init_db()
    maestro = JobExecutioner(args.config)

    if args.env:
        env_vars = parse_env_vars(args.env)
        if env_vars:
            for job_id, job in maestro.jobs.items():
            ...
            maestro.logger.info(f"Command-line environment variables: {list(env_vars.keys())}")

    if args.sequential:
        maestro.parallel = False
        maestro.logger.info("Parallel execution disabled")
    elif args.parallel:
        maestro.parallel = True
        maestro.logger.info("Parallel execution enabled")
    if args.workers is not None and args.workers > 0:
        maestro.max_workers = args.workers
        maestro.logger.info(f"Set workers to {args.workers}")
